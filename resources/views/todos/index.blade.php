...
(() => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (action === 'delete') {
                    ...
                    // Update todo status instantly
                    const isCompleted = action === 'complete';
                    updateTodoStatus(todoItem, todoId, isCompleted);
                }
            } else {
                const errorData = await response.json().catch(() => ({}));
                showMessage(errorData.message || 'Action failed', 'error');
            }
        } catch {
    ...
    function updateTodoStatus(todoItem, todoId, isCompleted) {
        ...
        title.classList.toggle('opacity-70', isCompleted);
        ...
        if (todoGrid) {
            todoGrid.innerHTML = `
                <div class="col-span-full text-center py-12">
                    <div class="text-6xl mb-4 text-white/30">
                        <i class="fas fa-clipboard-list"></i>
                    </div>
                    <h3 class="text-xl font-bold text-white mb-2">No todos found</h3>
                    <p class="text-white/60 mb-6">Start organizing your tasks and boost productivity!</p>
                    <a href="{{ route('todos.create') }}" 
                       class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-cyan-500 to-purple-500 text-white font-medium rounded-xl hover:opacity-90 transition-opacity">
                        <i class="fas fa-plus-circle"></i>
